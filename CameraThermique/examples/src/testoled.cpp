...
//    3. Pack the bit into the correct byte (page-addressed, matches hardware).
// =============================================================================
...
// =============================================================================
//  OLED display, no upscale — places the raw SRC_W×SRC_H grid 1:1 at the
//  top-left of the panel (same page-addressed packing as oledUpscale above).
//  Everything outside the SRC_W×SRC_H footprint stays off (0/black).
// =============================================================================
    ...
    for (int oy = 0; oy < SRC_H; oy++) {
        for (int ox = 0; ox < SRC_W; ox++) {
            if (src[oy][ox] >= mid) {
                ...
                const int bitIdx  = (oy % 8);          // LSB = topmost row of page
...
int main()
{
    ...
    while (running) {
        ...
        if (!paused) {
            ...
            // oledDisplayNoUpscale(oriented, oledBuf, minT, maxT);   // alt: no upscale, top-left
