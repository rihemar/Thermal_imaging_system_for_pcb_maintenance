...
{   int8_t minVal=src[0][0];
   int8_t maxVal=src[0][0];
    memset(dst, 0, OLED_H * OLED_W);
    for (int i=0;i<SRC_H;i++){
            for (int j=0;j<SRC_W;j++){
                if(minVal>src[i][j])minVal=src[i][j];
                if(maxVal<src[i][j])maxVal=src[i][j];
...
static void oledDisplayNoUpscale(const float src[SRC_H][SRC_W],
                         ...
                         float      minVal, float maxVal)
{
    ...
    // Horizontal format: each byte = 8 horizontally adjacent pixels, MSB = leftmost
    // byte index = row * (OLED_W / 8) + col / 8
    // bit  index = 7 - (col % 8)   <-- MSB is leftmost pixel
...
static void arrayToVector(const float src[SRC_H][SRC_W],
                         uint8_t    dst[SRC_H * SRC_W ],
                         float      minVal, float maxVal)
{
    memset(dst, 0, OLED_H * OLED_W / 8);
...
static uint8_t           orie[OLED_H*OLED_W];  // vertically-flipped sensor data
...
int main()
{
    ...
    MLX90640_SetRefreshRate  (MLX_I2C_ADDR, 0b001);  // 2 Hz
    ...
    while (running) {
        ...
        if (!paused) {
            ...
            // arrayToVector(oriented, orie, minT, maxT);
            oledUpscale2(oriented, orie);
            // oledDisplayNoUpscale(oriented,oledBuf,minT,maxT);
            myOLED.OLEDBitmap(0, 0, OLED_W, OLED_H, orie, false);
            myOLED.OLEDupdate();
